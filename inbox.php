...
if (isset($_GET['read'])) {
    ...
    $stmt = $conn->prepare("UPDATE inbox SET is_read = TRUE WHERE id = :id AND recipient_id = :user_id");
...
if (isset($_GET['view'])) {
    ...
    if ($viewMessage && !$viewMessage['is_read']) {
        $conn->prepare("UPDATE inbox SET is_read = TRUE WHERE id = :id")->execute([':id' => $messageId]);
    }
}
